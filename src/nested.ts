...
export function sumPublishedPoints(questions: Question[]): number {
    ...
    return publishedQuestions.reduce(
        (currentSum: number, publishedQuestion: Question): number =>
            currentSum + publishedQuestion.points,
        0,
    );
}
...
export function toCSV(questions: Question[]): string {
    const stringQuestions: string = questions
        .map(
...
export function duplicateQuestionInArray(
    ...
    return questions.reduce(
        (newQuestions: Question[], question: Question): Question[] =>
            question.id === targetId ?
                [...newQuestions, question, duplicateQuestion(newId, question)]
